  ...
  <el-dialog :append-to-body="true" :close-on-click-modal="false" :before-close="cancel" :visible.sync="dialog" :title="isAdd ? '新增' : '编辑'" width="900px">
    <el-form ref="form" :model="form" :inline="true" :rules="rules" size="small" label-width="80px">
      ...
      <el-form-item  label="商品品牌">
         <el-select v-model="form.shopBrandDto.id" filterable :filter-method="dataFilter" clearable placeholder="选择商品品牌">
            <el-option v-for="(item,index) in brand" :value="item.id" :key="index" :label="item.brandName"></el-option>
          </el-select>
      </el-form-item>
...
<script>
import { getCates } from '@/api/shopStoreCategory'
import { getBrand } from '@/api/shopBrand'
import { add, edit } from '@/api/shopStoreProduct'
...
export default {
  ...
  data() {
    return {
      loading: false, dialog: false, cates: [], brand: [],
      form: {
        id: '',
        merId: 0,
        image: '',
  ...
  methods: {
    ...
    getBrand(){
        getBrand({ enabled: true }).then(res => {
