<template>
  <div class="app-container">
    ...
    <!--表单组件-->
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu" :title="crud.status.title" width="500px">
    ...
    <el-table ref="table" v-loading="crud.loading" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-permission="['admin','SHOPSTORECATEGORY_EDIT','SHOPSTORECATEGORY_DELETE']" label="操作" width="130px" align="center" fixed="right">
        <template slot-scope="scope">
          <udOperation
            :data="scope.row"
...
<script>
import crudDept from '@/api/shopStoreCategory'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '分类', url: 'api/shopStoreCategory', sort: 'sort,desc', crudMethod: { ...crudDept }})
...
export default {
  ...
  data() {
    return {
      ...
      permission: {
        add: ['admin', 'SHOPSTORECATEGORY_CREATE'],
        edit: ['admin', 'SHOPSTORECATEGORY_EDIT'],
        del: ['admin', 'SHOPSTORECATEGORY_DELETE']
      },
      enabledTypeOptions: [
